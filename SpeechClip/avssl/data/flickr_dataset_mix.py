...
class FlickrDatasetMIX(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        wav_base_path_en = os.path.join(self.dataset_root, "flickr_audio", wav_file)
        wav_base_path_ar = os.path.join(self.dataset_root, "flickr_audio_ar", wav_file)

        print("wav_base_path: ", wav_base_path_en)
        wav_names_to_paths = defaultdict(list)
        #wav_list_en = os.listdir(wav_base_path_en)
        with open (os.path.join(self.dataset_root,"flickr_audio/english_audio.pickle"),'rb') as pick:
          wav_list_en = pickle.load(pick)

        wav_names = {p[:-6] for p in wav_list_en if p.split(".")[-1] == "wav"}
        ...
        with open (os.path.join(self.dataset_root,"flickr_audio_ar/arabic_audio.pickle"),'rb') as pick:
          wav_list_ar = pickle.load(pick)

        wav_names = {p[:-6] for p in wav_list_ar if p.split(".")[-1] == "wav"}
        ...
        show_list= list(wav_names_to_paths.values())
        for i in range(10):
          print("length: ", len(show_list[i]))
          print("i: ", show_list[i])
