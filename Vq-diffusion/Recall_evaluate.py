    ...
    def __getitem__(self, index):
        ...
        if  self.split == 'gen':   
            if  self.data_dir.find('places') != -1:   
                ...
                img_path = '%s/%s_%d.png' % (self.data_dir, key,num)
                img_path = img_path.replace('/content/drive/MyDrive/Final_Trials/mixed/cub/SpeechClip/data/CUB_200_2011/images/','').replace('.jpg','') 
                #print(img_path,"imaaaaaaaaaaage")
            ...
            if  self.data_dir.find('places') != -1:   
                ...
                img_path = key#'%s/images/%s.jpg' % (self.data_dir, key)  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', dest='data_dir', type=str, default='/data/CUB-200') #/tudelft.net    
    ...
    if args.data_dir.find('Flickr8k') == -1:
        file_path = 'evaluation/birds/filenames.pickle'
    else:
    ...
    ground_loader = torch.utils.data.DataLoader(
        dataset_ground, batch_size=7,
        drop_last=True, shuffle=False)
    gen_loader = torch.utils.data.DataLoader(
        dataset_gen, batch_size=7,
        drop_last=True, shuffle=False)
    ...
    for batch in tqdm(ground_loader):
        img,clss = batch
    ...
    ground_cls = torch.cat(ground_cls).cuda()
    ...
    gen_cls = torch.cat(gen_cls).cuda()
    ...
    print("gen_cls", gen_cls.is_cuda)
    print("indx_i2a", indx_I2A.is_cuda)
