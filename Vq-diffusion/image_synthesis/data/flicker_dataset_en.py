...
class FlickerDataset(Dataset):
    def __init__(self, data_root,input_file, phase = 'train', im_preprocessor_config=None, drop_caption_rate=0.0):
        ...
        self.image_folder = data_root#os.path.join(data_root, 'Images')
        ...
        if phase == "train":
          embedding_filename = os.path.join(data_root, 'speech/speech_embeddings_train.pickle')
        else:
          embedding_filename = os.path.join(data_root, 'speech/speech_embeddings_test.pickle')
        with open(embedding_filename, 'rb') as f:
            self.embeddings = pickle.load(f, encoding="bytes")
